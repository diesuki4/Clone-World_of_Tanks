...
public class LSJ_DriveControlInput01KeyboardStepwise : LSJ_DriveControlInput00Base
{
    ...
    {
        // Set "vertical".
        if (Input.GetKeyDown(LSJ_GeneralSettings.Drive_Up_Key) && speedStep < forwardStepCount)
        { // Forward
            speedStep += 1;
            controlScript.Shift_Gear(speedStep);
        }
        else if (Input.GetKeyDown(LSJ_GeneralSettings.Drive_Down_Key) && speedStep > -reverseStepCount)
        { // Backward
            speedStep -= 1;
            controlScript.Shift_Gear(speedStep);
        }
        else if (Input.GetKeyDown(LSJ_GeneralSettings.Drive_Brake_Key) && speedStep != 0)
        { // Stop
            speedStep = 0;
            controlScript.Shift_Gear(speedStep);
        }
