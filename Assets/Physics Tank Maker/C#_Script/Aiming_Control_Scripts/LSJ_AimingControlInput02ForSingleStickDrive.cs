...
public class LSJ_AimingControlInput02ForSingleStickDrive : LSJ_AimingControlInput01MouseKeyboard
{
    ...
    public override void Get_Input()
    {
        ...
        if (Input.GetKeyDown(LSJ_GeneralSettings.Aim_Mode_Switch_Pad_Button))
        {
        ...
        { // The gun camera is enabled now.
            ...
            aimingScript.Adjust_Angle.x += Input.GetAxis("Horizontal2") * LSJ_GeneralSettings.Aiming_Sensibility * multiplier;
            aimingScript.Adjust_Angle.y += Input.GetAxis("Vertical2") * LSJ_GeneralSettings.Aiming_Sensibility * 0.5f * multiplier;

            // Check it is locking-on now.
            if (aimingScript.Target_Transform)
            { // Now locking-on the target.
              // Cancel the lock-on.
                if (Input.GetKeyDown(LSJ_GeneralSettings.Turret_Cancel_Pad_Button))
                {
            ...
            { // Now not locking-on.
              // Try to find a new target.
                if (Input.GetKey(LSJ_GeneralSettings.Turret_Cancel_Pad_Button) == false)
                {
        ...
        { // The gun camera is not enabled now.
            ...
            if (Input.GetKey(LSJ_GeneralSettings.Turret_Cancel_Pad_Button))
            {
