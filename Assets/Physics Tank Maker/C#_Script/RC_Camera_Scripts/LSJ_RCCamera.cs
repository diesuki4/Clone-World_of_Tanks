...
public class LSJ_RCCamera : MonoBehaviour
{
    /*
    ...
    */
    ...
    protected LSJ_RCCameraInput00Base inputScript;
    ...
    protected virtual void Set_Input_Script(int type)
    {
        switch (type)
        {
            ...
            case 1: // Mouse + Keyboard (Pressing)
                inputScript = gameObject.AddComponent<LSJ_RCCameraInput01Mouse>();
                break;

            case 2: // Gamepad (Single stick)
            case 4: // Gamepad (Triggers)
                inputScript = gameObject.AddComponent<LSJ_RCCameraInput02ForSingleStickDrive>();
                break;

            case 3: // Gamepad (Twin sticks)
                inputScript = gameObject.AddComponent<LSJ_RCCameraInput03ForTwinSticksDrive>();
                break;
        }
    }
    ...
    bool Find_Target()
    {
        ...
        if (LSJ_IDManager.Instance == null)
        {
            Debug.LogWarning("RC Camera cannot find the target. There is no 'ID_Manager_CS' in this scene.");
            return false;
        }

        for (int i = 0; i < LSJ_IDManager.Instance.ID_Scripts_List.Count; i++)
        {
            if (LSJ_IDManager.Instance.ID_Scripts_List[i].Is_Selected)
            {
                targetIDScript = LSJ_IDManager.Instance.ID_Scripts_List[i];
                var cameraScript = targetIDScript.GetComponentInChildren<LSJ_CameraPointsManager>();
        ...
        var targetGunCameraScript = targetTransform.GetComponentInChildren<LSJ_GunCamera>();
