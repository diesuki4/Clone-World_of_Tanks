...
public class LSJ_StaticTrackSetting : MonoBehaviour
{
    /*
    ...
    */
    ...
    void Set_Type()
    {
        ...
        var hitColliders = Physics.OverlapSphere(thisTransform.position, 0.1f, LSJ_LayerSettings.Layer_Mask);
        foreach (Collider hitCollider in hitColliders)
        {
            ...
            if (hitColliderParent.GetComponent<LSJ_CreateRoadWheel>() || hitColliderParent.GetComponent<LSJ_CreateRoadWheelType89>())
            ...
            if (hitColliderParent.GetComponent<LSJ_CreateSprocketWheel>() || hitColliderParent.GetComponent<LSJ_CreateIdlerWheel>() || hitColliderParent.GetComponent<LSJ_CreateSupportWheel>())
    ...
    void Set_Child_Scripts(LSJ_StaticTrackParent parentScript)
    { // Called from "Create_TrackBelt_CSEditor".

        // Add "Static_Track_Piece_CS" and set the values.
        var pieceScript = gameObject.AddComponent<LSJ_StaticTrackPiece>();
        ...
        {
            case 0: // Static
                if (frontTransform.GetComponent<LSJ_StaticTrackSetting>().type == 1)
                { // The front piece is Anchor type.
                  // Change to Dynamic type.
                    pieceScript.Type = 2;
                }
                else if (rearTransform.GetComponent<LSJ_StaticTrackSetting>().type == 1)
        ...
        if (Use_2ndPiece)
        {
            var switchScript = gameObject.AddComponent<LSJ_StaticTrackSwitchMesh>();
        ...
        var damageScript = GetComponent<LSJ_DamageControl03PhysicsTrackPiece>();
        if (damageScript)
        {
            DestroyImmediate(damageScript);
        }
        var stabilizerScript = GetComponent<LSJ_Stabilizer>();
        if (stabilizerScript)
        {
        ...
        for (int i = 0; i < transform.childCount; i++)
        {
            ...
            if (childObject.layer == LSJ_LayerSettings.Reinforce_Layer)
            { // Reinforce.
    ...
    { // Called from "Create_TrackBelt_CSEditor".
      // Set the "Front_Script" and "Rear_Script" in the "Static_Track_Piece_CS".
        var pieceScript = gameObject.GetComponent<LSJ_StaticTrackPiece>();
        pieceScript.Front_Script = pieceScript.Front_Transform.GetComponent<LSJ_StaticTrackPiece>();
        pieceScript.Rear_Script = pieceScript.Rear_Transform.GetComponent<LSJ_StaticTrackPiece>();
    }
