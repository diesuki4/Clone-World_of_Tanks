...
public class LSJ_IDManager : MonoBehaviour
{
    /*
    ...
    */
    ...
    public List<LSJ_IDSettings> ID_Scripts_List = new List<LSJ_IDSettings>(); // Referred to from "RC_Camera_CS".

    public static LSJ_IDManager Instance;
    ...
    int Get_Proper_ID(LSJ_IDSettings idScript)
    {
        // Check the ID.
    ...
    void Select_Tank()
    {
        ...
        if (Input.GetKeyDown(LSJ_GeneralSettings.Select_Default_Tank_Key))
        {
        ...
        if (Input.GetKeyDown(LSJ_GeneralSettings.Increase_ID_Key))
        {
            for (int i = 0; i < usedIDList.Count; i++)
            {
                currentID += 1;
        ...
        if (Input.GetKeyDown(LSJ_GeneralSettings.Decrease_ID_Key))
        {
            for (int i = 0; i < usedIDList.Count; i++)
            {
                currentID -= 1;
