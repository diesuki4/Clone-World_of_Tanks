...
public class LSJ_AI : MonoBehaviour
{
    /*
    ...
    */
    ...
    public LSJ_AISettings Settings_Script; // Referred to from "AI_Headquaters_CS".
    ...
    bool GunsCanAim;
    LSJ_BulletGenerator[] bulletGeneratorScripts;
    float castRayCount;
    bool hasApproached;

    // For driving.
    LSJ_AIHand handScript;
    ...
    void Initialize()
    {
        ...
        Settings_Script = GetComponentInParent<LSJ_AISettings>();
        ...
        handScript = thisTransform.GetComponentInChildren<LSJ_AIHand>();

        // Get all the "Bullet_Generator_CS" in the tank.
        bulletGeneratorScripts = thisTransform.parent.GetComponentsInChildren<LSJ_BulletGenerator>();
        ...
        {
            LSJ_UIAIStateControl stateScript = Settings_Script.AI_State_Text.GetComponent<LSJ_UIAIStateControl>();
            if (stateScript == null)
            {
                stateScript = Settings_Script.AI_State_Text.gameObject.AddComponent<LSJ_UIAIStateControl>();
            }
        ...
        {
            LSJ_AIShareTarget shareTargetScript = gameObject.AddComponent<LSJ_AIShareTarget>();
    ...
    void Search_Target()
    {
        ...
        { // The target is already dead.
            ...
            {
                LSJ_AIHeadquaters.Instance.Give_Order();
            }
            return;
        }
    ...
    bool Cast_Line_For_Searching(Vector3 targetPosition)
    {
        ...
        if (Physics.Linecast(eyeTransform.position, targetPosition, out raycastHit, LSJ_LayerSettings.Layer_Mask))
        {
    ...
    bool Set_Can_Aim()
    {
        ...
        for (int i = 0; i < bulletGeneratorScripts.Length; i++)
        {
            ...
            RaycastHit raycastHit;
            if (Physics.Linecast(bulletGeneratorScripts[i].transform.position, aimingScript.Target_Position, out raycastHit, LSJ_LayerSettings.Layer_Mask))
            {
                if (raycastHit.transform.root == targetRootTransform)
    ...
    public void Set_Target(LSJ_AIHeadquatersHelper targetAIHelperScript)
    ...
    public bool Check_for_Assigning(LSJ_AIHeadquatersHelper targetAIHelperScript)
    { // Called from "AI_Headquaters_CS" in the scene.
      // Check this tank can detect the target or not, by casting a line.
        ...
        RaycastHit raycastHit;
        if (Physics.Linecast(eyeTransform.position, tempTargetPosition, out raycastHit, LSJ_LayerSettings.Layer_Mask))
