...
public class LSJ_EventEvent01SpawnTank : LSJ_EventEvent00Base
{
    ...
    {
        // Check the tank around the spawning point.
        Collider[] colliders = Physics.OverlapSphere(transform.position, 5.0f, LSJ_LayerSettings.Detect_Body_Layer_Mask);
        ...
        for (int i = 0; i < colliders.Length; i++)
        {
            ...
            { // The tank is already dead.
                var respawnScript = colliders[i].GetComponentInParent<LSJ_RespawnController>();
                if (respawnScript)
                {
    ...
    void Spawn_Tank()
    {
        ...
        var idScript = newTank.GetComponent<LSJ_IDSettings>();
        if (idScript)
        {
        ...
        var respawnScript = newTank.GetComponent<LSJ_RespawnController>();
        if (respawnScript)
        {
        ...
        var specialSettingsScript = newTank.GetComponent<LSJ_SpecialSettings>();
        ...
        var aiSettingsScript = newTank.GetComponent<LSJ_AISettings>();
        if (aiSettingsScript)
        {
