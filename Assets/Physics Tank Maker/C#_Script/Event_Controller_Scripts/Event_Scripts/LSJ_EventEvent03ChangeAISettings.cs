...
public class LSJ_EventEvent03ChangeAISettings : LSJ_EventEvent00Base
{
    ...
    public override void Execute_Event()
    {
        for (int i = 0; i < eventControllerScript.Target_Tanks.Length; i++)
        {
            ...
            var aiSettingsScript = eventControllerScript.Target_Tanks[i].GetComponentInChildren<LSJ_AISettings>();
            if (aiSettingsScript)
