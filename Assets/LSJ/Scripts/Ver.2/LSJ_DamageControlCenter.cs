...
public class LSJ_DamageControlCenter : MonoBehaviour
{
    /* 
    ...
    */
    ...
    void Turret_Destroyed(int index)
    {
        ...
        {
            Instantiate(Turret_Props[index].destroyedEffect, Turret_Props[index].turretBaseTransform.position, Turret_Props[index].turretBaseTransform.rotation, Turret_Props[index].turretBaseTransform);
        }
        ...
        if (Turret_Props[index].blowOff == true)
        {
            ...
            turretRigidbody.AddForceAtPosition(Turret_Props[index].turretBaseTransform.up * Random.Range(Turret_Props[index].mass * 5.0f, Turret_Props[index].mass * 15.0f), Turret_Props[index].turretBaseTransform.position + addForceOffset, ForceMode.Impulse);
    ...
    void Track_Destroyed(bool isLeft)
    {
        ...
        if (Repairable_Track)
        {
            ...
            if (GetComponentInChildren<LSJ_StaticTrackParent>() || GetComponentInChildren<LSJ_TrackScroll>())
            {
    ...
    { // Called from "ID_Settings_CS".
        ...
        if (LSJ_UIHPBarsSelf.Instance)
        {
            LSJ_UIHPBarsSelf.Instance.Get_Damage_Script(this);
        }
    }
